...
// this define may need to be changed into ATTinyCore/hardware/avr/1.5.2/variants/tinyX5/pins_arduino.h
// experimental feature: https://github.com/SpenceKonde/ATTinyCore/pull/205
...
// FUDGE_FACTOR for timing retrocompability
const uint8_t FUDGE_FACTOR = 8; //old timing where 1 LSB for us; so we will have to divide all by this amout

//ms are in ms
const uint8_t ms_loopTime = 15; //duty cycle falling edge to falling edge
const uint16_t us_timeOut = 30000; //timeout between pulses
...
const uint16_t us_max_high_pulse = 2000; //maximum lenght of a high pulse
...
uint16_t us_maxFwd = 250; //2000
uint16_t us_neutral = 187; //1500
uint16_t us_maxRev = 125; //1000
uint16_t us_radOff = 187; //1493
...
int getPulseDuration(){
  ...
  while (!intNewRead){
    if (micros() - startTimeout >= us_timeOut){
      startTimeout += us_timeOut; //make sure this function return at TIMEOUT_PULSE
      // timeout!
      return 0;
    }
  }
  ...
  if (lastDuration <= us_max_high_pulse){
    return lastDuration / FUDGE_FACTOR;
  }else{
    // timeout!
    return 0;
  }
...
void evalSpeed(uint8_t us_pulseTime){
  static uint8_t no_revTaps = 0;
  static bool flag_rev = false;
  static int ms_revTimer = 0; //needs to go negative
  static uint8_t no_timeOutCount = 0;
